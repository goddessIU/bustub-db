...
class LockManager {
 public:
  ...
  auto IsCompatible(std::shared_ptr<LockRequestQueue> &queue, const table_oid_t &oid, LockMode lock_mode) -> bool;
  //  auto IsCompatibleForRow(const std::shared_ptr<LockManager::LockRequestQueue>& queue, const table_oid_t &oid,

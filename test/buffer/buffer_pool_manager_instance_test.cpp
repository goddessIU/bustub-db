...
TEST(BufferPoolManagerInstanceTest, ConcurrentTest1) {
  const int num_runs = 50;
  ...
  for (int run = 0; run < num_runs; run++) {
    ...
    for (int tid = 0; tid < num_threads; tid++) {
      //      page_id_t page_id_temp;
      threads.emplace_back([tid, &bpm]() { EXPECT_EQ(true, bpm->UnpinPage(tid, true)); });
    }
    ...
    for (int tid = 0; tid < num_threads; tid++) {
      //      page_id_t page_id_temp;
      threads.emplace_back([tid, &bpm]() { bpm->FetchPage(tid); });
    }
    ...
    for (int tid = 0; tid < num_threads; tid++) {
      //      page_id_t page_id_temp;
      threads.emplace_back([tid, &bpm]() { EXPECT_EQ(true, bpm->UnpinPage(tid, false)); });
    }
    ...
    for (int tid = 0; tid < num_threads; tid++) {
      threads.emplace_back([&bpm, &vec_]() {
        page_id_t page_id_temp;
        bpm->NewPage(&page_id_temp);
        vec_.push_back(page_id_temp);
      });
    }
    ...
    for (int tid = 0; tid < num_threads; tid++) {
      threads.emplace_back([tid, &bpm, &vec_]() { bpm->DeletePage(vec_[tid]); });
    }
...
TEST(BufferPoolManagerInstanceTest, Test2) {
  const std::string db_name = "test.db";
...
TEST(BufferPoolManagerInstanceTest, DISABLED_Test1) {
  const std::string db_name = "test.db";
  const size_t buffer_pool_size = 10;
  const size_t k = 5;
  std::vector<page_id_t> vec;
  ...
  for (size_t d = 0; d < k; d++) {
    for (int i = vec.size() - 1; i >= 0; --i) {
      EXPECT_EQ(nullptr, bpm->FetchPage(vec.at(i)));
    }
  }
  // Scenario: After unpinning pages {0, 1, 2, 3, 4} and pinning another 4 new pages,
  // there would still be one buffer page left for reading page 0.
  for (int i = 0; i < 5; ++i) {
    EXPECT_EQ(true, bpm->UnpinPage(vec.at(i), true));
  }
  for (int i = 0; i < 5; ++i) {
    EXPECT_EQ(false, bpm->DeletePage(vec.at(i)));
  }
  for (int i = 0; i < 5; ++i) {
    EXPECT_EQ(nullptr, bpm->FetchPage(vec.at(i)));
  }
